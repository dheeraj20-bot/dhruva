"use client";

import Image from "next/image";
import React from "react";
import { CardBody, CardContainer, CardItem } from "./ui//3d-card";
import Link from "next/link";

interface Project {
  title: string;
  description: string;
  imageUrl: string;
  websiteUrl: string;
}

const projects: Project[] = [
  {
    title: "Ghamedia",
    description: "Mixture of Framer Motion Animation and Next js",
    imageUrl:"/gha.jpeg",
    websiteUrl: "https://ghamedia.com",
  },
  {
    title: "Amy Laing Coaching",
    description: "Mixture of Framer Motion Animation and Next js",
    imageUrl:"/amy.jpeg",
    websiteUrl: "https://amylaingcoaching.com",
  },
  {
    title: "DentSpa",
    description: "Mixture of Framer Motion Animation and Next js",
    imageUrl: "https://images.unsplash.com/photo-1441974231531-c6227db76b6e?q=80&w=2560&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    websiteUrl: "https://ghamedia.com",
  },
 
];

export function ThreedCard() {
  return (
    <section className="grid max-w-7xl mx-auto grid-cols-1  sm:grid-cols-3 gap-4">
      {projects.map((project, index) => (
        <CardContainer key={index} className="w-full">
          <CardBody className="bg-gray-50 relative group/card  dark:hover:shadow-2xl dark:hover:shadow-emerald-500/[0.1] dark:bg-slate-950 dark:border-white/[0.2] border-black/[0.1] w-auto sm:w-[30rem] h-auto rounded-xl p-6 border  ">
            <CardItem
              translateZ="50"
              className="text-3xl font-bold text-neutral-600 dark:text-white"
            >
              {project.title}
            </CardItem>
            <CardItem
              as="p"
              translateZ="60"
              className="text-neutral-500 text-lg max-w-sm mt-2 dark:text-neutral-300"
            >
              {project.description}
            </CardItem>
            <CardItem translateZ="100" className="w-full mt-4">
              <Image
                src={project.imageUrl}
                height="1000"
                width="1000"
                className="h-60 w-full object-cover rounded-xl group-hover/card:shadow-xl"
                alt="thumbnail"
              />
            </CardItem>
            <div className="flex justify-between items-center mt-20">
              <CardItem
                translateZ={20}
                as={Link}
                href={project.websiteUrl}
                target="__blank"
                className="px-4 py-2 rounded-xl text-xs font-normal dark:text-white"
              >
                Vist the website →
              </CardItem>
            </div>
          </CardBody>
        </CardContainer>
      ))}
    </section>
  );
}
